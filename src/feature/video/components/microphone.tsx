...
const MicrophoneButton = (props: MicrophoneButtonProps) => {
  ...
  return (
    <div className={classNames('microphone-footer', className)}>
      {isStartedAudio ? (
        <Tooltip title={`${isStartedAudio ? 'stop camera' : 'start camera'}`}>
        <Button
          className={classNames('vc-button', className)}
          icon={isMuted ? ( audio === 'phone' ? ( <IconFont type="icon-phone-off" /> ) : ( <AudioMutedOutlined /> ) ) : audio === 'phone' ? ( <IconFont type="icon-phone" /> ) : ( <AudioOutlined /> )}
          ghost={true}
          shape="circle"
          size="large"
